...
def main():
    ...
    counts = log_analyzer(logfilepath)

    print("IP addresses with failed login attempts:")
    for ip_address in counts:
        print(f"{ip_address} has {counts[ip_address]} failed login attemps.")

    print("Suspicious IP addresses that have more than 3 failed login attempts:")
    suspicious_found = False
    for ip_address in counts:
        if counts[ip_address] >= 3:
            print(f"{ip_address} has {counts[ip_address]} failed login attemps and is flagged for suspicious activity")
